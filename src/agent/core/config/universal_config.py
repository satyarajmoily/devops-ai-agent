...
class UniversalConfigLoader:
    ...
    def _load_yaml(self, filename: str) -> Dict[str, Any]:
        ...
        try:
            with open(config_path, 'r') as f:
                ...
                expanded_config = self._expand_environment_variables(config or {})
    ...
    def _expand_environment_variables(self, config: Any) -> Any:
        ...
        if isinstance(config, dict):
            ...
            # Look for environment variable patterns like ${VAR_NAME} or ${VAR_NAME:-default}
            def replace_env_var(match):
                ...
                if ':-' in var_expression:
                    ...
                    env_value = os.environ.get(var_expression)
                    if env_value is None:
                        logger.warning(f"Environment variable '{var_expression}' not found, keeping original value")
    ...
    def get_llm_config(self, agent_name: Optional[str] = None) -> Dict[str, Any]:
        """Get LLM configuration from agent-specific settings in agents.yml"""
        if agent_name is None:
            # Try to detect agent from environment or default to devops-ai-agent
            agent_name = os.environ.get('AGENT_NAME', 'devops-ai-agent')
        
        # Get agent configuration
        agent_config = self.get_agent_config(agent_name)
        
        # Extract LLM config from environment variables
        env_vars = agent_config.get('environment', [])
        llm_config = {}
        
        # Parse environment variables to extract LLM config
        for env_var in env_vars:
            if '=' in env_var:
                key, value = env_var.split('=', 1)
                if key.startswith('LLM_'):
                    # Convert environment variable name to config key
                    config_key = key[4:].lower()  # Remove 'LLM_' prefix and lowercase
                    
                    # Convert string values to appropriate types
                    if config_key in ['temperature']:
                        llm_config[config_key] = float(value)
                    elif config_key in ['max_tokens', 'timeout']:
                        llm_config[config_key] = int(value)
                    else:
                        llm_config[config_key] = value
        
        # Add API keys from platform credentials
        platform_credentials = self.platform_config.get("credentials", {}).get("llm", {})
        llm_config.update({
            "openai_api_key": platform_credentials.get("openai_api_key"),
            "anthropic_api_key": platform_credentials.get("anthropic_api_key")
        })
        
        if not llm_config.get("provider") or not llm_config.get("model"):
            raise ValueError(f"LLM configuration incomplete for agent '{agent_name}'. Check agents.yml configuration.")
        
        return llm_config
    
    def get_agent_llm_config(self, agent_name: str) -> Dict[str, Any]:
        """Get LLM configuration for specific agent"""
        return self.get_llm_config(agent_name)
    
    def get_llm_credentials(self) -> Dict[str, Any]:
        """Get only LLM API credentials from platform.yml"""
